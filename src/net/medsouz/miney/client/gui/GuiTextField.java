...
public class GuiTextField extends Gui {
	...
	public void writeText(String par1Str) {
		...
		int j = this.cursorPosition < this.selectionEnd ? this.selectionEnd
				: this.cursorPosition;
		int k = this.maxStringLength - this.text.length()
				- (i - this.selectionEnd);
		//boolean flag = false;
	...
	public void mouseClicked(int par1, int par2, int par3) {
		boolean flag = par1 >= this.xPos && par1 < this.xPos + this.width
		...
		if (this.isFocused && par3 == 0) {
			...
			String s = this.fontRenderer
					.trimStringToWidth(
							...
							this.getWidth());
			this.setCursorPosition(this.fontRenderer.trimStringToWidth(s, l)
	...
	public void drawTextBox() {
		if (this.getVisible()) {
			...
			int i1 = this.enableBackgroundDrawing ? this.yPos
			...
			if (s.length() > 0 && flag && j < s.length()) {
				this.fontRenderer.drawStringWithShadow(s.substring(j), j1, i1,
						i);
			}
			...
			if (k != j) {
				int l1 = l
						+ this.fontRenderer.getStringWidth(s.substring(0, k));
				this.drawCursorVertical(k1, i1 - 1, l1 - 1, i1 + 1
	...
	public void setSelectionPos(int par1) {
		...
		if (this.fontRenderer != null) {
			...
			String s = this.fontRenderer.trimStringToWidth(
					this.text.substring(this.lineScrollOffset), k);
	...
	public void setTextWrapWidth(int x){
		textWrapWidth = x;
	}
	
	public void wrapText(){
		if(wrapText)
			wrappedText = TextTools.wordWrap(text, textWrapWidth);
	}
	
	public void enableTextWrap(boolean enable){
		wrapText = enable;
	}
}
